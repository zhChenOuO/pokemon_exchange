...
func (s *service) CreateSpotOrder(ctx context.Context, data *model.SpotOrder) error {
	if err := s.repo.CreateSpotOrder(ctx, nil, data); err != nil {
		log.Error().Msgf("fail to create spot order %+v", err)
		return err
	}

	s.spotOrderChan[data.CardID] <- data
	return nil
}

func (s *service) AsyncMatchingSpotOrder(ctx context.Context, data *model.SpotOrder) error {
	var (
